...
use crate::rsa::{RsaSigner, RsaVerifier};
...
#[must_use]
pub fn sign_service(client_secret: impl AsRef<[u8]>, parts: &ServiceStringToSign<'_>) -> Signature {
    HmacSigner::new(client_secret).sign(parts.build())
}
...
/// private RSA key.
pub fn sign_oauth(signer: &RsaSigner, parts: &OAuthStringToSign<'_>) -> Signature {
    signer.sign(parts.build())
}

/// Verify a SNAP BI OAuth signature using the partner's public RSA key.
pub fn verify_oauth(verifier: &RsaVerifier, parts: &OAuthStringToSign<'_>, sig: &Signature) -> Result<()> {
