...
fn the_significant_digits_available_at_each_magnitude() {
    ...
    for &(exponent, units, expected) in rows {
        ...
        assert!(Rate::<USD, IDR>::from_units(units).is_some(), "1e{exponent} must be representable");
    }
}

/// **The contract's number was right.** A rate of `1e-13` holds six digits, so the seventh is
/// not merely imprecise — it does not exist, and `from_units` cannot round to it.
#[test]
...
fn the_smallest_representable_rate_still_converts() {
    let smallest = Rate::<USD, IDR>::from_units(1).expect("1e-18 is in domain");

    // A big enough amount survives: 1e18 units * 1e-18 = 1 unit.
    let big = Money::<USD>::from_units(POW10_SCALE).expect("in domain");
    ...
    let small = Money::<USD>::from_units(POW10_SCALE - 1).expect("in domain");
...
fn both_directions_of_a_hyperinflation_pair_are_representable() {
    ...
    let forward = Rate::<USD, IDR>::from_units(3_000_000 * POW10_SCALE).expect("in domain");
    // Reverse: 1/3e6 = 3.3333...e-7, which at scale 18 is 333_333_333_333 units.
    let reverse = Rate::<IDR, USD>::from_units(333_333_333_333).expect("in domain");
    ...
    let start = Money::<USD>::from_major(1).expect("in domain");
