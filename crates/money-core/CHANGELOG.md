...
- Point extension integration guidance to the standalone kamu-money-pg repository.
- Remove test guidance that assumed extension builds shared this workspace.
...
- `AmountError::WideOutOfDomain` carries the exact total, as the 32
  little-endian bytes `advanced::UnitSum::to_le_bytes` produces, when a sum is
  refused for being wider than `i128`.

  `UnitSum::finish` previously answered that case with the value-less
  `ArithmeticOverflow`, discarding a total the accumulator was holding at that
  moment. Refusing an amount that leaves the domain is the contract; forgetting
  what it was is not, and the case is reachable rather than theoretical — 171
  terms at the domain edge exceed `i128`.

  `ArithmeticOverflow` remains, and still carries nothing, for the two places
  that genuinely have nothing to report: `UnitSum::add_units` and
  `UnitSum::merge` overflowing the accumulator itself, which no sequence of
  in-domain terms can reach and only a forged transition state produces.

  Adding a variant to a `#[non_exhaustive]` enum, so existing matches keep
  compiling.
