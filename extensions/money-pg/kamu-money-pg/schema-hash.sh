...
#
# What IS stable is the SET of objects. This strips the two provenance comments pgrx embeds --
#
...
# needs an oracle too -- "64 tests still pass" proves the ones that ran are green, not that the
# same 64 exist -- and this is it.
FEATURES="${3:-}"
...
if ! cargo pgrx schema "pg${PG}" "${FEATURE_ARGS[@]}" --out "$WORK/schema.sql" >"$WORK/gen.log" 2>&1; then
